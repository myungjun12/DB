...
--COMPANY 시퀀스 생성 
-- 시퀀스명 COMPANY_SEQ 시작값 1 NOCACHE NOCYCLE
-- 방법 1
...
-- SNACKS_SEQ 시퀀스 생성 
-- 시퀀스명 SNACKS_SEQ 시작값 1 증가값1 NOCACHE NOCYCLE
...
-- 배배로 1100 2
-- 조고파이 2000 2
INSERT INTO SNACKS VALUES(company_seq.nextval,'오에스',1500,1);
INSERT INTO SNACKS VALUES(company_seq.nextval,'칸조',1220,1);
INSERT INTO SNACKS VALUES(company_seq.nextval,'배배로',1100,2);
INSERT INTO SNACKS VALUES(company_seq.nextval,'조고파이',2000,2);
...
SELECT *
FROM CONPANY
WHERE CONPANY_ADDRESS = '서울시 강남구';
-- 회사 ID가 1인 과자 조회 -- SNACKS
SELECT *
FROM SNACKS
WHERE COMPANY_ID = 1;
-- 회사 ID가 1DLRH, 그 회사의 과자들 조회 (JOIN)
SELECT C.COMPANY_NAME,S.SNACK_NAME
FROM COMPANY C
