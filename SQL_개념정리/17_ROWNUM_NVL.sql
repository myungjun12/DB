...
FROM EMPLOYEE -- 1
WHERE ROWNUM <= 3 -- 2 현재 입력된 표의 1,2,3에 있는 급여에 대해 정렬되기 때문에 
ORDER BY SALARY DESC; -- 마지막 실행 되기 때문에

--2.만약에 제대로 나온 값이 아니라면 수정 (SUBQUERY)SUBQUERY = 1에서 조회한 쿼리

SELECT ROWNUM,EMP_NAME,SALARY 
FROM (SELECT EMP_ID,EMP_NAME,SALARY FROM EMPLOYEE ORDER BY SALARY DESC) 
-- 전체 직원의 월급을 기준으로 나열 시키고 
WHERE ROWNUM <= 3; -- 1,2,3 번째 사람에 대해
