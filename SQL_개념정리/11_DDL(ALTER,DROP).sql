...
-- 제약조건 추가 : ALTER TABLE 테이블명
                    --  ADD [CONSTRAINT 제약조건명] 제약조건(컬럼명)[REFERENCE 테이블명(컬럼명)];

--DEPARTMENT 테이블을 복사해서 예제 진행   

CREATE TABLE DEPT_COPY AS SELECT * FROM DEPARTMENT; -- 테이블 복사
...
-----> 데이터타입 VARCHAR2(100)  ->  VARCHAR2(10) 데이터 타입이 10 초과되는건 불가

-- 컬럼 삭제 : ALTER TABLE 테이블명 DROP(삭제할 컬럼명);
-->          ALTER TABLE 테이블명 DROP COLUMN 삭제할컬럼명;

--> 테이블에는 최소 1개 이상의 컬럼이 존재해야하기 때문에 모든 컬럼을 삭제할 수는 없음
...
----> 컬럼값을 조절할 때 (큰 값 -> 작은 값) 이미 저장된 데이터값이 변경할 값보다 크지 않은지 확인 
...
--ALTER TABLE DEPT_COPT DROP COLUMN CNAME;
/* 
00942. 00000 -  "table or view does not exist"
만약에 존재하지 않는 컬럼을 삭제하려하면 나오는 ERROR
*/
...
ALTER TABLE DEPT_COPY RENAME TO DCOPY; -- 테이블 이름 변경
