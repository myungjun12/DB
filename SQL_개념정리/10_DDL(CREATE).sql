...
CREATE TABLE "USER_MEMBER"(
              USER_ID VARCHAR2(20),
              USER_PW형 VARCHAR2(20),
              USER_NAME VARCHAR2(15),
              USER_SSN CHAR(14),
              ENROLL_DATE DATE DEFAULT SYSDATE
              );
-- DEFAULT(=기본값) 컬럼의 기본값 지정 (필수 아님)    
--> DEFAULT 기본값을 지정하고 INSRT나 UPDATE 문을 작성해줄 때
---> 지정한 기본값이 들어감
...
-- [표현식]
-- COMMENT ON COLUMN 테이블명.컬럼명 IS '주석내용'; -> 주석내용 "" 사용불가
...
CREATE TABLE UNIQUE_TABLE(
  ...
  --USER_ID VARCHAR2(20) UNIQUE, -- 컬럼 레벨 제약 조건 (이름 X)
    --(방법 1)
    USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE, -- CONSTRAINT 제약조건명 제약조건 종류
                                                      -- 컬럼 레벨 제약 조건 (이름 O) 
    --(방법 2)
    USER_NAME VARCHAR2(30), -- 테이블 레벨로 UNIQUE 제약 조건 설정
    
    -- 테이블 레벨 --
    -- UNIZUE(USER_NAME) -- 컬럼 레벨 제약 조건 (이름 X)
    CONSTRAINT USER_NAME_U UNIQUE(USER_NAME)
    );

-- CAFE_ID NUMBER 10 기본키(PRIMARY KEY)
-- CAFE_NAME VARCHAR2(100) 널값 못들어가게 설정 
-- CAFE_PHONE VARCHAR2(20) 핸드폰번호 카페 중복 X(방법 1)
-- CAFE_ADDRESS VARCHAR2(100) 주소 카페 중복 X (방법 2)
...
-- 위의 테이블 값 설정에서 PHONE VARCHAR(2)가 자리 부족해서 오류떴는데
-- ALTER 사용해서 PHONE VARCHAR의 크기를 변경했다.
COMMIT;
...
--UNIQUE : 컬럼에서 NULL이 한개는 허용됨 두개 이상 안됨 
--PRIMARY KEY : NULL값 안되고 중복안됨
...
/*
ORA-02291: integrity constraint (KH_T.G_C_F) violated - parent key not found
참조한 부모에 40이 없어서 값을 추가할 수 없음
*/

INSERT INTO USER_GRADE VALUES(40,'플래티넘회원');
-->부모키인 USER_GRADE에 40이라는 값이 존재하기 때문에 삽입가능해짐

INSERT INTO USER_FK VALUES (5,'USER05','PW05','50');
-- parent key not found

INSERT INTO USER_GRADE VALUES(00,'임시회원');-->정수에서 맨앞에 0은 사라짐[NUMBER값설정되어있음]
...
----- 코드 10을 참조하는 자식이 있으므로 삭제할 수 없음

-- 배달 어플 A카페 폐업
--- 배달 어플 -> A카페 카페 마음대로 폐업은 못하지
---- A카페가 열어 놓은 메뉴들은 모두 삭제하고 가
----- A카페 폐업할 때 연결된 메뉴까지 한 번에 삭제하는게 있음

--CASCADE -- 부모키 삭제시 참조하는 자식의 행도 모두 삭제됨
...
/*
ORA-02290: check constraint (KH_T.SYS_C007289) violated
...
-- 2번 방법 따로 index에 기록한다음 조건 설정한 경우 (한 줄 작성)
--컬럼명 자료형 CONSTRAINT 인덱스에기록할이름 CHECK(컬럼명 IN('조건1','조건2'));

-- 3번 방법 따로 index에 기록한다음 조건 설정한 경우 (여러 줄 작성)
--컬럼명 자료형, 
