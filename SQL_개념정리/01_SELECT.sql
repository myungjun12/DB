...
/*
WHERE OR 사용해서 값1, 값2, 값3, ... 과 같은 표시를 진행하기 도 함
...
FROM 테이블명
WHERE 조건식 
ORDER BY 컬럼명|별칭| ASC|DESC

컬럼순서의 기본 값은 오름차순
오름차순 : ASC (ASCending의 약자)
내림차순 : DESC(DESCending의 약자)
...
FROM EMPLOYEE 
WHERE DEPT_CODE ='D5','D6','D9'
ORDER BY DEPT_CODE; -- 오름차순이 기본값
...
FROM EMPLOYEE 
WHERE SALARY BETWEEN 3000000 AND 6000000
ORDER BY 2 DESC; -- 2 = EMP_NAME (SELECT 옆의 순번)
...
FROM EMPLOYEE
ORDER BY SALARY DESC; -- 월급 * 12달 = 연봉 내림차순 조회
...
SELECT EMP_NAME,SALARY * 12 AS "연봉"
FROM EMPLOYEE 
ORDER BY "연봉" DESC; --SALARY * 12 대신 "연봉" 작성할 수 있음

-- 주의 할 점 : ORDER BY 에서는 별칭 사용이 가능하지만 
-- WHERE 절의 경우 조건이기 때문에 별칭 사용이 불가능
-- ORDER BY는 결과를 가지고 결과 정리를 하는 표현방법 
-- WHERE 은 결과를 나타내기 위해 찾는 조건문이기 때문 
-- 조건이 진행도 안됐는데 별칭부터 붙인 것이기 때문
...
D1 부서에서는 직급코드 J7이 J6 보다 먼저 작성되고, 
그다음 J7은 단독이기 때문에 정렬할 수 있는게 없음
J6에서는 2명 이상이 존재하기 때문에 J6 직급의 직원들의 이름이 오름차순 정렬
