...
컬럼에서만 가능
* 널이 아님에 체크를 해주지 않은 컬럼에서는 가능함

* 널이 아님 체크를 해준 컬럼 : 필수로 값을 넣어햐하는 공간

예를 들어 
회원가입 
아이디 이름 이메일 번호 주소  → 널이 아님 체크(필수 입력)
기념일 → 널이 아님 체크 해지 (null값 가능)

INSERT 에서 값을 선택해서 넣을 수 있는 컬럼이라면 
선택해서 넣을 수 있는 컬럼명을 지워준 후(작성하지 않은 후)
VALUES 다음에 값을 집어 넣는 공간에도 
값을 추가하지 않으면 됨 
*/
/* 공백이나 띄어쓰기는 관계 없이 원하는 SQL 구문 만큼 드래그해서 실행 */
INSERT INTO kh_cafe (c_id, c_name, c_address, c_phone, c_time) 
VALUES (2, '빠나쁘레쏘', '서울시 강남구 역삼동',
'02-5555-5555', '영업시간 : 12:00 ~ 14:00');

INSERT INTO kh_cafe (c_id, c_name, c_address, c_phone, c_time)
VALUES (3, '검포즈커피', '경기도 화남시 화남구', '031-3232-3333', '영업시간 평일 15:00 ~ 18:00');
...
-- 카페에서 카페이름 카페번호 보는데 AS를 사용해서 카페이름 카페번호 표기해서 보여주기
SELECT * FROM kh_cafe;
